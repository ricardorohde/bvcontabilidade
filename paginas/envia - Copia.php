...
else if($op == "rescisao")
{
	...
	*/
	...
	$msg = '<table cellpadding="10" cellspacing="0" width="100%" border="0">
			...
			<tr><td>A 1&ordf; parcela do 13º&deg; Sal&aacute;rio j&aacute; foi paga?:</td><td>'.$form['parcela13_1'].'</td></tr>
			<tr><td>A 2&ordf; parcela do 13º&deg; Sal&aacute;rio j&aacute; foi paga?:</td><td>'.$form['parcela13_2'].'</td></tr>
			<tr><td colspan="2"><hr /></td></tr>
...
.texto{
	...
	if($form['tipoaviso'] == "indenizado")
	{
		...
		$mensagem .= '<h2 style="margin-bottom: 50px; text-align: center;">AVISO PRÉVIO INDENIZADO</h2>
				...
				$dados = explode("|",@$form['sindicato']);
				$datasind = strtotime(str_replace("/","-",$dados[0]));
				$id_sind = $dados[1];
				
				if($data_aviso > $datasind && $id_sind == 30)
				{
