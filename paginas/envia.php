...
else if($op == "rescisao")
{
	...
	*/
	...
	{
		$data = date("Y-m-d",dataTimestamp($data));
		return $data;
	}
	function calculoData($dataini, $datafim, $periodo)
	{
		$date = new DateTime( dataAmericano($dataini) ); 
		$interval = $date->diff( new DateTime( dataAmericano($datafim) ) );
		$data = $interval->format( '%'.$periodo );
		return $data;
	}
	...
	$num_anos = calculoData($form['data_admissao'],$form['data_demissao'],"Y");
	
	
	$dia_demissao = date("d",dataTimestamp($form['data_demissao']));
	if($dia_demissao == "31")
		$dia_demissao = 30;
		
	$mes_admissao = date("m",dataTimestamp($form['data_admissao']));
	$mes_demissao = date("m",dataTimestamp($form['data_demissao']));
	...
	if($form['tipoaviso'] == "trabalhado")
	{
		$data_projecao = date("Y-m-d",strtotime("+".$total_diasaviso." days",$data_demissao));
		$data_dissidio2 = date("Y-m-d",$data_dissidio);
		
		$dia_convencao = strtotime($data_dissidio2);
		$dia_limite = strtotime($data_projecao);

		$diferenca = $dia_convencao - $dia_limite;
	...
	if($num_anos < 1)
	{
		...
		if($ano_admissao == $ano_demissao)
		{
			...
			$date = new DateTime( $data_admissao2 ); 
			$interval = $date->diff( new DateTime( $data_demissao2 ) );
			$dia_demissao2 = $interval->format( '%d' );
        ...
        <tr>
        	<td colspan="2">
            	<table class="tblhoras" cellpadding="5" cellspacing="0" width="100%">
					...
					{
						$data = explode("-",$form['dia'][$indice]);
						$numsemana = date("W",strtotime((str_replace("/","-",($data[0]."-".date("Y"))))));
